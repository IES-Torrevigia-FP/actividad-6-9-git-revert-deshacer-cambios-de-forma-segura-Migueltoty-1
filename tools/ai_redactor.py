...
Extracto Reflexion 6.9 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.9) se califica con rubrica: Git revert (max 4), Gestion de conflictos (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece revert_checks.cp2_revert_simple.ok en true, reconoce explicitamente el revert de un commit concreto.
- Si en JSON aparece revert_checks.cp3_revert_head.ok en true, reconoce explicitamente el uso de `git revert HEAD`.
- Si en JSON aparece revert_checks.cp4_revert_multiple_no_commit.ok en true, reconoce explicitamente el uso de `git revert --no-commit`.
- Si en JSON aparece revert_checks.cp5_conflicto_revert.ok en true, reconoce explicitamente la resolucion de conflicto con `git revert --continue`.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-9.md'))
